...
func newHTTPPassthroughPipeline(coreConfig model.Reader, eventPlatformReceiver eventplatformreceiver.Component, desc passthroughPipelineDesc, destinationsContext *client.DestinationsContext, pipelineID int) (p *passthroughPipeline, err error) {
	...
	for i, endpoint := range endpoints.GetReliableEndpoints() {
		...
		reliable = append(reliable, logshttp.NewDestination(endpoint, desc.contentType, destinationsContext, http.NewLimitedMaxSenderPool(endpoints.BatchMaxConcurrentSend), true, destMeta, pkgconfigsetup.Datadog(), pipelineMonitor))
	}
	additionals := []client.Destination{}
	for i, endpoint := range endpoints.GetUnReliableEndpoints() {
		destMeta := client.NewDestinationMetadata(desc.eventType, pipelineMonitor.ID(), "unreliable", strconv.Itoa(i))
		additionals = append(additionals, logshttp.NewDestination(endpoint, desc.contentType, destinationsContext, http.NewLimitedMaxSenderPool(endpoints.BatchMaxConcurrentSend), false, destMeta, pkgconfigsetup.Datadog(), pipelineMonitor))
	}
