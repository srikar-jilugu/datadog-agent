...
type mockManager struct {
	trapsPackets, trapsPacketsUnknownCommunityString, trapsBlaBla int64
	lock                                                          sync.Mutex
	err                                                           error
}
...
func (s *mockManager) AddTrapsBlaBla(i int64) {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.trapsBlaBla += i
}

func (s *mockManager) GetTrapsBlaBla() int64 {
	s.lock.Lock()
	defer s.lock.Unlock()
	return s.trapsBlaBla
}
