...
func (c *collector) getImageMetadata(ctx context.Context, imageID string, newSBOM *workloadmeta.SBOM) (*workloadmeta.ContainerImageMetadata, error) {
	...
	imageInspect, err := c.dockerUtil.ImageInspect(ctx, imageID)
	if err != nil {
		log.Warnf("error getting image inspect: %s", err)
	}
	...
	return &workloadmeta.ContainerImageMetadata{
		...
		Layers:       layersFromDockerHistoryAndInspect(imageHistory, imageInspect),
		SBOM:         sbom,
	}, nil
}
