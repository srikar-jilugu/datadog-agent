...
func (t *TaggerWrapper) EnrichTags(tb tagset.TagsAccumulator, originInfo origindetection.OriginInfo) {
	...
	case origindetection.ProductOriginDogStatsDLegacy:
		...
		if originInfo.LocalData.ProcessID != 0 && (originInfo.LocalData.PodUID == "" || !t.datadogConfig.dogstatsdEntityIDPrecedenceEnabled) {
			containerID, err := t.GenerateContainerIDFromProcessID(originInfo, metaCollector)
			if err != nil {
	...
	default:
		...
		if originInfo.LocalData.ProcessID != 0 {
			generatedContainerID, err := t.GenerateContainerIDFromProcessID(originInfo, metaCollector)
			if err != nil {
				t.log.Tracef("Failed to resolve container ID from process ID %d: %v", originInfo.LocalData.ProcessID, err)
			}
