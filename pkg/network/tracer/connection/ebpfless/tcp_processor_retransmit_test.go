...
// testAllRetransmits tries retransmitting every outgoing packet in a capture and makes
// sure the outcome is consistent (same stats plus one retransmit)
func testAllRetransmits(t *testing.T, packets []testCapture, expectedStats network.StatCounters) {
	// make one subtest retransmitting each outgoing packet
	for i, pkt := range packets {
		// only outgoing retransmits are considered
		if pkt.pktType != unix.PACKET_OUTGOING {
			continue
		}

		nextSeq := calcNextSeq(pkt.tcp, pkt.payloadLen())
		// filter out ack packets that don't increase the sequence number since they can't be retransmitted
		if nextSeq-pkt.tcp.Seq == 0 {
			continue
		}

		t.Run(fmt.Sprintf("retransmit packet number %d", i), func(t *testing.T) {
			var traffic []testCapture
			// include the ith packet twice
			traffic = append(traffic, packets[:i]...)
			traffic = append(traffic, packets[i])
			traffic = append(traffic, packets[i:]...)
...
// TestRetransmitBasicHandshake runs through each possible retransmit on an outgoing connection
func TestRetransmitBasicHandshake(t *testing.T) {
	...
	testAllRetransmits(t, basicHandshake, expectedStats)
}

// TestRetransmitReversedBasicHandshake runs through each possible retransmit on an incoming connection
func TestRetransmitReversedBasicHandshake(t *testing.T) {
	pb := newPacketBuilder(lowerSeq, higherSeq)
	basicHandshake := []testCapture{
