...
func TestBatchOffsetRace(t *testing.T) {
	o := newOffsetManager(4)
	batch1 := &Batch{Idx: 1, Len: 10}
	batch2 := &Batch{Idx: 1, Len: 2}

	o.Get(0, batch1, false)
	begin, end := o.Get(0, batch2, false)

	require.GreaterOrEqual(t, end, begin, "Offset calculation should never be negative")
}
