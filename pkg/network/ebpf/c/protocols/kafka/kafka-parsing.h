...
static __always_inline bool kafka_process(conn_tuple_t *tup, kafka_info_t *kafka, pktbuf_t pkt, kafka_telemetry_t *kafka_tel) {
    ...
    */
    ...
    __u8 version =  (__u8)kafka_header.api_version;
    if (version > 0) {
        if (kafka_header.api_key == KAFKA_PRODUCE && version < KAFKA_CLASSIFICATION_MAX_SUPPORTED_PRODUCE_REQUEST_API_VERSION) {
            __sync_fetch_and_add(&kafka_tel->classified_produce_api_version_hits[version], 1);
        } else if (kafka_header.api_key == KAFKA_FETCH && version < KAFKA_CLASSIFICATION_MAX_SUPPORTED_FETCH_REQUEST_API_VERSION) {
            __sync_fetch_and_add(&kafka_tel->classified_fetch_api_version_hits[version], 1);
        }
    }
