...
func (h *RemoteConfigHandler) mrfUpdateCallback(updates map[string]state.RawConfig, applyStateCallback func(string, state.ApplyStatus)) {
	...
	if len(updates) == 0 {
		log.Debugf("no MRF configuration in remote config update payload")
		return
	}

	var enableAPM *bool
	var enableAPMCfgPth string
	for cfgPath, update := range updates {
		if enableAPM != nil && *enableAPM {
			break
		}
	...
	if enableAPM != nil {
		pkglog.Infof("Setting `multi_region_failover.failover_apm: %t` through remote config", *enableAPM)
		h.agentConfig.IsMRFEnabled = func() bool {
			return *enableAPM
		}
