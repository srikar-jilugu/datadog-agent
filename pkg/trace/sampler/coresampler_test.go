...
func TestSamplerAccessRace(t *testing.T) {
	...
	statsdClient.EXPECT().Count(metricSamplerSeen, gomock.Any(), serviceSignature.metricTags(), float64(1)).MinTimes(loopCount).MaxTimes(totalCount)
	statsdClient.EXPECT().Count(metricSamplerKept, gomock.Any(), serviceSignature.metricTags(), float64(1)).MinTimes(loopCount).MaxTimes(totalCount)
