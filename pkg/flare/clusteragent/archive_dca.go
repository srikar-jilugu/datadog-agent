...
func getDCAAutoscalerList() ([]byte, error) {
	...
	taggerListURL := fmt.Sprintf("https://%v:%v/autoscaler-list", ipcAddress, pkgconfigsetup.Datadog().GetInt("cluster_agent.cmd_port"))

	return flare.GetAutoscalerList(autoscalerListURL)
}
