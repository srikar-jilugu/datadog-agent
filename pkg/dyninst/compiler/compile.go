...
func CompileBPFProgram(program ir.Program) error {
	generatedCode := bytes.NewBuffer(nil)
	codegen.GenerateCode(logical.EncodeProgram(program), generatedCode)
	...
	injector := func(in io.Reader, out io.Writer) error {
		...
		fmt.Println(generatedCode.String())
		err = t.Execute(out, generatedCode.String())
		return err
	}
