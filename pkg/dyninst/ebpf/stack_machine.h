...
extern const uint64_t enqueue_op_data_len;
extern const uint32_t enqueue_op_max_len;
extern const uint8_t enqueue_op_data[];
...
extern type_t lookup_go_subroutine(uint64_t pc);
extern bool get_type_info(type_t t, const type_info_t** info_out);

bool chased_pointer_contains(chased_pointers_t* chased, target_ptr_t ptr,
                             type_t type) {
  if (!chased) {
    return false;
  }
...
sm_read_program_uint8(stack_machine_t* sm) {
  uint8_t param = enqueue_op_data[sm->pc];
  sm->pc += 1;
  return param;
}
...
sm_read_program_uint16(stack_machine_t* sm) {
  uint32_t param = read_uint16(&enqueue_op_data[sm->pc]);
  sm->pc += 2;
  return param;
}
...
sm_read_program_uint32(stack_machine_t* sm) {
  uint32_t param = read_uint32(&enqueue_op_data[sm->pc]);
  sm->pc += 4;
  return param;
}
...
sm_call_enqueue_for_type(stack_machine_t* sm, const type_info_t* t) {
  ...
  if (t->enqueue_pc >= enqueue_op_data_len) {
    LOG(1,
        "chase: enqueue_pc out of "
        "bounds %ld >= %ld",
        t->enqueue_pc, enqueue_op_data_len);
    return false;
  }
  if (sm->pc_stack_pointer >= ENQUEUE_STACK_DEPTH) {
...
extern type_t lookup_go_interface(uint64_t go_runtime_type);
...
static long sm_loop(__maybe_unused unsigned long i, void* _ctx) {
  ...
  // bounds checks.
  if (sm->pc >= enqueue_op_data_len - enqueue_op_max_len) {
  ...
  if (sm->pc >= enqueue_op_data_len - enqueue_op_max_len + 1) {
    return 1;
  }
  switch (op) {
