...
class TestDownloadWithRetry(unittest.TestCase):
    ...
    def test_download_log(self, gh_mock):
        ...
        download_with_retry(download_function=download_logs, run=run, destination='.', retry_count=3, retry_interval=0)
    ...
    def test_downloads_artifacts_with_repository(self, gh_mock):
        ...
        gh_mock.return_value = self.gh
        download_with_retry(
            download_function=download_artifacts, run=run, destination='.', retry_count=3, retry_interval=0
        )
    ...
    def test_connection_error(self, gh_mock, print_mock):
        ...
        with self.assertRaises(Exit):
            download_with_retry(
                download_function=download_logs, run=run, destination='.', retry_count=2, retry_interval=0
            )
    ...
    def test_other_error(self, gh_mock, print_mock):
        ...
        with self.assertRaises(LookupError):
            download_with_retry(
                download_function=download_logs, run=run, destination='.', retry_count=1, retry_interval=0
            )
